...
[![code coverage](https://zjonsson.github.io/node-@hishpr/optio-quisquam/badge.svg)](https://zjonsson.github.io/node-@hishpr/optio-quisquam/) 

[npm-image]: https://img.shields.io/npm/v/@hishpr/optio-quisquam.svg
[npm-url]: https://npmjs.org/package/@hishpr/optio-quisquam
[travis-image]: https://api.travis-ci.org/ZJONSSON/node-@hishpr/optio-quisquam.png?branch=master
[travis-url]: https://travis-ci.org/ZJONSSON/node-@hishpr/optio-quisquam?branch=master
[downloads-image]: https://img.shields.io/npm/dm/@hishpr/optio-quisquam.svg
[downloads-url]: https://npmjs.org/package/@hishpr/optio-quisquam
[coverage-image]: https://3tjjj5abqi.execute-api.us-east-1.amazonaws.com/prod/node-@hishpr/optio-quisquam/badge
[coverage-url]: https://3tjjj5abqi.execute-api.us-east-1.amazonaws.com/prod/node-@hishpr/optio-quisquam/url

# @hishpr/optio-quisquam
...
```bash
$ npm install @hishpr/optio-quisquam
```
...
fs.createReadStream('path/to/archive.zip')
  .pipe(@hishpr/optio-quisquam.Extract({ path: 'output/path' }));
```
...
```js
fs.createReadStream('path/to/archive.zip')
  .pipe(@hishpr/optio-quisquam.Parse())
...
```js
const zip = fs.createReadStream('path/to/archive.zip').pipe(@hishpr/optio-quisquam.Parse({forceStream: true}));
...
If you `pipe` from @hishpr/optio-quisquam the downstream components will receive each `entry` for further processing.   This allows for clean pipelines transforming zipfiles into unzipped data.
...
```js
async function main() {
  const directory = await @hishpr/optio-quisquam.Open.file('path/to/archive.zip');
...
const @hishpr/optio-quisquam = require('./unzip');

async function main() {
  const directory = await @hishpr/optio-quisquam.Open.url(request,'http://www2.census.gov/geo/tiger/TIGER2015/ZCTA5/tl_2015_us_zcta510.zip');
...
async function getFile(req, res, next) {
  const directory = await @hishpr/optio-quisquam.Open.url(request, googleStorageOptions);
...
async function main() {
  const directory = await @hishpr/optio-quisquam.Open.s3(s3Client,{Bucket: '@hishpr/optio-quisquam', Key: 'archive.zip'});
  return new Promise( (resolve, reject) => {
    directory.files[0]
      .stream()
      .pipe(fs.createWriteStream('firstFile'))
...
async function main() {
  const directory = await @hishpr/optio-quisquam.Open.buffer(buffer);
  console.log('directory',directory);
  // ...
}
...
async function main() {
  ...
  const directory = await @hishpr/optio-quisquam.Open.custom(customSource);
  console.log('directory', directory);
  // ...
}
